...
angularApp.directive("searchResults", function(){
    ...
    return{
        ...
        // Compile function runs once and mostly we won't do any html manipulation. So compile function is empty and angularJS provides the option for this.
        // instead of having "compile", you can directly use the link function alone with "link:". Please refer the below commented at the bottom
        compile: function(elem, attrs){
            console.log("Compiling...");
            console.log(elem);
            console.log(elem.html());

            // We can on the fly at the run time manipulate the html content
            //for example removing the all class attributes in the searchresults.html content
            //elem.removeAttr('class');
            //console.log(elem);
            //console.log(elem.html());

            // returning the linkage object
            return{
                // Linking runs as many times customs directive runs
                // pre linking
                // Avoid using pre link as per the angularJS documentation
                ...
                // post linking
                // difference between pre-link and post-link: Consider you are using the custom directive within custom directives like <search-results> <search-results> </search-results> </search-results>
                // first pre-link runs for first <search-results> and then pre-link for second <search-results>, so all the pre-link will run first.
                // After all the pre-link ran, post-link run for first <search-results>, and them second <search-results>, Since all the pre-links are running first, it is always better to use any html manipulation in the post-link function
                post: function(scope, elements, attrs){
                    ...
                    //Example of manipulating the VIEW based on some conditions of the MODEL data
